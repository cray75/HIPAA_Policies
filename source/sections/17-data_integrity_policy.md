...
Consensus Networks takes data integrity very seriously. As stewards and partners of Consensus Networks Customers, we strive to assure data is protected from unauthorized access and that it is available when needed. The following policies drive many of our procedures and technical settings in support of the Consensus Networks mission of data protection.
...
1. All access to Production Systems must be logged. This is done following the Consensus Networks Auditing Policy.

## 17.4 Prevention of Malware on Production Systems

1. All Production Systems must have Snort running, and set to scan system every 2 hours and at reboot to assure no malware is present. Detected malware is evaluated and removed.
...
3. All Production Systems are to only be used for Consensus Networks business needs.
...
4. Ensure Consensus Networks Customer Production Data is segmented and only accessible to Customers authorized to access data.
5. All Production Data at rest is stored on encrypted volumes using encryption keys managed by Consensus Networks. Encryption at rest is ensured through the use of automated deployment scripts referenced in the [Configuration Management Policy](#9.-configuration-management-policy).
...
1. All data transmission is encrypted end to end using encryption keys managed by Consensus Networks. Encryption is not terminated at the network end point, and is carried through to the application.
