...
Consensus Networks standardizes and automates configuration management through the use of Ansible Playbooks as well as documentation of all changes to production systems and networks. Ansible automatically configure all Consensus Networks systems according to established and tested policies, and are used as part of our Disaster Recovery plan and process.
...
2. No systems are deployed into Consensus Networks environments without approval of the Consensus Networks executive team.
3. All changes to production systems, network devices, and firewalls are approved by the Consensus Networks executive team before they are implemented to assure they comply with business and security requirements.
...
6. Tooling to generate an up-to-date inventory of systems, including corresponding architecture diagrams for related products and services, is hosted on GitLab.
   ...
   * These scripts are used to generate the diagrams and asset lists required by the Risk Assessment phase of Consensus Networks' Risk Management procedures ([§4.3.1](#4.3-risk-management-procedures)).
...
10. Consensus Networks utilizes development and staging environments that mirror production to assure proper function.
11. Consnesus Networks also deploys environments locally using Vagrant to assure functionality before moving to staging or production.
12. All formal change requests require unique ID and authentication.
13. Consensus Networks uses the [Security Technical Implementation Guides (STIGs)](http://iase.disa.mil/stigs/) published by the Defense Information Systems Agency as a baseline for hardening systems.
...
1. Before provisioning any systems, ops team members must file a request in the Consensus Networks Quality Management System.
   * Quality Management System access requires authenticated users.
   * The executive team grants access to the Quality Management System following the procedures covered in the [Access Establishment and Modification section](#7.2-access-establishment-and-modification).
2. The executive team, must approve the provisioning request before any new system can be provisioned.
...
4. If the system will be used to house production data (ePHI), the ops team member must add an encrypted block data volume to the VM during provisioning.
   ...
   * For systems on other cloud providers, the ops team member must add a block data volume and set up OS-level data encryption using Ansible.
...
7. The new system may be rotated into production once the executive team verifies all the provisioning steps listed above have been correctly followed and has marked the Issue with the `Approved` state.

### 9.3.1 Provisioning Linux Systems

1. Linux systems have their baseline security configuration applied via ansible playbooks. These baseline playbooks cover:
...
2. Any additional Ansible Playbooks applied to the Linux system must be clearly documented by the ops team member in the DT request by specifying the purpose of the new system.

### 9.3.2 Provisioning Windows Systems

1. Windows systems have their baseline security configuration applied via the combination of Group Policy settings and Ansible Playbooks. These baseline settings cover:
...
2. Provisioning the first Ansible server for a production pod requires bootstrapping Ansible. An authorized member of the Dev Ops team will oversee provisioning a new Ansible server.
   * Once the Ansible server has been bootstrapped, the ops team member will apply the baseline configuration to the Ansible server.
3. Critical infrastructure services such as logging, monitoring, LDAP servers, or Windows Domain Controllers must be configured with appropriate Ansible Playbooks.
   * These Ansible Playbooks have been approved by the executive team, or an authorized delegate of the executive team, to be in accordance with all Consensus Networks policies, including setting appropriate:
...
2. Configuration changes to Ansible Playbooks must be initiated by creating a Merge Request in GitLab.
   ...
   * At least one other ops team member must review the Ansible change before merging the change into the main branch.
...
4. Once the request has been approved by the executive team, the ops team member may roll out the change into production environments.

## 9.5 Patch Management Procedures

1. Consensus Networks uses automated tooling to ensure systems are up-to-date with the latest security patches.
...
4. Code reviews are performed as part of the pull request procedure. Once a change is ready for review, the author(s) will notify other engineers using an appropriate mechanism, typically via an `@channel` message in Keybase.
   * Other engineers will review the changes, using the guidelines above.
   * Engineers should note all potential issues with the code; it is the responsibility of the author(s) to address those issues or explain why they are not applicable.
5. If the feature or defect interacts with ePHI, or controls access to data potentially containing ePHI, the code changes must be reviewed by two members of Consensus Networks' Software Security Team before the feature is marked as complete.
   ...
   * Software Security Team members are required to undergo annual training on identifying the most common software vulnerabilities and will receive ongoing training on Consensus Networks' compliance and security requirements.
