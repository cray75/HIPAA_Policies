...
1. Requests for access to Consensus Networks HealthNet Platform systems and applications are made formally using the following process:
  1. A Consensus Networks workforce member initiates the access request by creating an Issue in the Consensus Networks Quality Management System.
...
5. Any Consensus Networks workforce member can request change of access using the process outlined in [§7.2 paragraph 1](#7.2-access-establishment-and-modification).
...
8. In the case of non-personal information, such as generic educational content, identification and authentication may not be required. This is the responsibility of Consensus Networks Customers to define, and not Consensus Networks.
...
11. Generic accounts are not allowed on Consensus Networks systems.
...
3. Consensus Networks maintains a minimum necessary approach to access to Customer data. As such, Consensus Networks, including all workforce members, does not readily have access to any ePHI.

## 7.4 Access Authorization

1. Role based access categories for each Consensus Networks system and application are pre-approved by the Security Officer, or an authorized delegate of the Security Officer.
2. Consensus Networks utilizes hardware and software firewalls to segment data, prevent unauthorized access, and monitor traffic for denial of service attacks.
...
3. All Customer support desk interactions must be verified before Consensus Networks support personnel will satisfy any request having information security implications.
   * Consensus Networks' current support desk software, Jira, requires users to authenticate before submitting support tickets.
   * Support issues submitted via Consensus Networks' dashboard require that users authenticate with their HealthNet account before submitting support tickets.
   * Support issues submitted by email must be verified by Consensus Networks personnel using a phone number that has been registered with the corresponding account.

## 7.6 Unique User Identification

1. Access to the Consensus Networks HealthNet Platform systems and applications is controlled by requiring unique User Login IDs and passwords for each individual user and developer.
...
6. Automated log-on configurations that store user passwords or bypass password entry are not permitted for use with Consensus Networks workstations or production systems.
...
All workstations at Consensus Networks are company owned, and all are laptop products running Mac OSX, Windows, or Linux.
...
9. All workstations are to have the following messages added to the lock screen and login screen: *This computer is owned by Consensus Networks LLC. By logging in, unlocking, and/or using this computer you acknowledge you have seen, and follow, these policies (**<insert_link_here>**) and have completed this training (**<insert_link_here>**). Please contact us if you have problems with this - info@consensusnetworks.com.*

## 7.9 Wireless Access Use

1. Consensus Networks production systems are not accessible directly over wireless channels.
...
4. Wireless networks managed within Consensus Networks non-production facilities (offices, etc.) are secured with the following configurations:
   * All data in transit over wireless is encrypted using WPA2 encryption;
   * Passwords are rotated on a regular basis, presently quarterly. This process is managed by the Consensus Networks Security Officer.
...
3. The Security Officer will terminate users' access rights immediately upon notification, and will coordinate with the appropriate Consensus Networks employees to terminate access to any non-production systems managed by those employees.
...
Consensus Networks does not use paper records for any sensitive information. Use of paper for recording and storing sensitive data is against Consensus Networks policies.

## 7.12 Password Management

1. User IDs and passwords are used to control access to Consensus Networks systems and may not be disclosed to anyone for any reason.
2. Users may not allow anyone, for any reason, to have access to any information system using another user's unique user ID and password.
3. On all production systems and applications in the Consensus Networks environment, password configurations are set to require:
...
12. In cases where a user has forgotten their password, the following procedure is used to reset the password.
    * The user submits a password reset request to info@consensusnetworks.com. The request should include the system to which the user has lost access and needs the password reset.
    ...
    * The administrator verifies the identity of the user either in-person or through a separate communication channel such as phone or Keybase.
...
2. Disallowing transfer of ePHI to workstations is enforced through technical measures.
   * All production access to systems is performed through a bastion/jump host accessed through a VPN. Direct access to production systems is disallowed by Consensus Networks' VPN configuration.
   ...
   * Configuration settings for enforcing these technical controls are managed by Consensus Network's configuration management tooling, Chef/Salt.

## 7.14 PaaS Customer Access to Systems

Consensus Networks grants HealthNet customer secure system access via VPN connections. This access is only to Customer-specific systems, no other systems in the environment. These connections are setup at customer deployment. These connections are secured and encrypted and the only method for customers to connect to Consensus Networks' hosted systems.

In the case of data migration, Consensus networks does, on a case by case basis, support customers in importing data. In these cases Consensus Networks requires that all data is secured and encrypted in transit, such as by using SFTP or SCP for transferring files.

In the case of an investigation, Consensus Networks will assist customers, at Consensus Networks's discretion, and law enforcement in forensics.
